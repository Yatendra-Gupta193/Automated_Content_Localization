...
from langdetect import detect, LangDetectException # importing Libraries
import os
from werkzeug.utils import secure_filename
import time
import uuid

# Workaround: some torchvision/transformers combinations expect
# `torch.utils._pytree.register_pytree_node`. On some torch builds
# the function is named `_register_pytree_node` instead — add a
# compatibility alias early to avoid import-time AttributeErrors.
try:
    import torch
    ...
    if pytree is not None:
        if not hasattr(pytree, "register_pytree_node") and hasattr(pytree, "_register_pytree_node"):
            setattr(pytree, "register_pytree_node", getattr(pytree, "_register_pytree_node"))
except Exception:
    # If torch isn't installed or the structure is different, continue
    pass

# Import local services after the compatibility shim so their imports
# don't trigger the known torchvision/transformers bug.
...
def process_voice():
    ...
    try:
        ...
        # Clean up the temporary converted speaker sample file to avoid reuse/caching
        try:
